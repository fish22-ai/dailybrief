...
所以每条卡片的解读是四段结构化内容，而不是一句摘要：
  ...
  chain  金融传导（8~12 环，**每一环都要写出机制**，不能只写结果）
...
链条里其余专业词全靠猜。所以这次把链条拉长到 8~12 环、把 term 扩成 notes 数组。
...
# max_tokens 太小会让每次输出都被截断、白烧重试；min_chain_hops 低于 7 就退回
# "看不懂"那个版本了（见 CLAUDE.md 第三节）。
...
MIN_CHAIN_HOPS = config.get_int("llm", "min_chain_hops", 7, lo=3, hi=30)
MIN_NOTES = config.get_int("llm", "min_notes", 3, lo=1, hi=12)
...
2. 每条给出四段解读，全部用中文。**读者是看得懂公式、但不熟悉行话缩写的学生**，\
所以宁可多写一句把机制说透，也不要写成结论清单。

   - what（发生了什么）：大白话一句话，谁做了什么、说了什么、数字是多少。\
不用行话缩写，不超过 60 字。
   - why（市场为什么在意）：说清这条**改变了市场原本在定价的哪个预期**，\
以及哪一类资产因此要重新估值。不超过 80 字。
   - chain（金融传导）：用 " → " 连接 **{{HOPS_LO}}~{{HOPS_HI}} 环**，\
从事件一路推到具体资产价格。
     · 每一环都必须写出**机制**，不能只写结果。反例："加息预期上升 → 成长股承压"\
（中间跳掉了无风险利率、折现率、现值三步）；正例："短期无风险利率 Rf 上移 → \
旧债固定票息相对新债性价比下降、投资者抛售 → 债券价格下跌、YTM 被动上行 → \
全市场定价基准抬高 → DCF 折现率 r 上行、远期现金流现值缩水 → 高估值成长股承压"。
...
担心违约，要求更高利息才肯借钱给企业。"
     ...
     · **只要这条涉及定价、折现、利差、收益率，就必须给公式并讲透公式怎么动**：\
先写公式，再说清"哪个变量动了、往哪个方向动、于是哪一项变大变小、结果是什么"。
     · 公式一律写成**纯文本并用反引号包住**，用 Unicode 符号（¹ ² ⁿ ₜ ᵗ ∑ ↑ ↓ ≈ −）。\
**禁止 LaTeX**：不要出现 \\frac、\\sum、\\( \\) 这类记法。照下面这样写：
       `P = C/(1+y)¹ + C/(1+y)² + … + (C+Face)/(1+y)ⁿ`
       `PV = ∑ CFₜ/(1+r)ᵗ`
       `Ri = Rf + β(Rm − Rf)`
...
{"id":7,
 ...
 "chain":"通胀持续高位 → 美联储官员释放鹰派表态、收紧货币政策预期升温 → 市场修正降息预期、开始定价未来加息概率上行 → 未来政策基准利率预期抬升 → 短期无风险利率 Rf 上移 → 旧美债固定票息相对新债性价比下降、投资者抛售短期美债 → 2 年期美债价格下跌、到期收益率 YTM 被动上行 → 全市场资产定价基准（无风险利率 Rf）整体抬高 → DCF 模型折现率 r 上行、远期现金流现值大幅缩水，同时 CAPM 中资产必要收益率 Ri 抬升、风险资产估值被压制 → 远期利润占比高、β 值大的高估值成长股价格明显承压；美国利率预期↑ → 美国资产相对吸引力↑ → 资金流向美元资产↑ → 美元可能走强",
 "notes":[
  "鹰派核心目标：强硬，把抑制通胀放在第一位，只要通胀高于目标就倾向加息、收紧货币政策。理念是物价稳定最重要 —— 就算加息导致经济放缓、失业率小幅上升，也是打压通胀可以接受的代价。",
  "鸽派核心目标：温和，保增长、保就业，对通胀有一定容忍度，不喜欢加息，偏好维持低利率放水刺激经济。",
  "加息存在时间滞后！今天宣布加息，不会明天物价就降下来，完整传导需要 6~12 个月。这就是通胀刚抬头时鹰派官员就呼吁赶紧加息、不能等物价飞上天再动手的原因。",
  "加息具体加的是什么？央行上调政策基准利率 → 银行之间借钱成本抬高 → 全社会贷款（房贷、消费贷、企业经营贷）利息跟着涨，同时存款利息提高 → 社会总需求 AD 下降。",
  "why 债券价格与收益率永远反向变动？债券定价公式 `P = C/(1+y)¹ + C/(1+y)² + … + (C+Face)/(1+y)ⁿ`。债券发行后票息 C 与本金 Face 都固定不变，市场预期利率上升时旧债吸引力不足、投资者卖出，价格 P 下跌；分子不变而 P 变小，只能是分母里的 y 变大 —— 所以 YTM 是被动抬升的。",
  "why 折现率能压制成长股估值？DCF 折现公式 `PV = ∑ CFₜ/(1+r)ᵗ`。成长股的现金流大量发生在远期、t 很大，而分母是 (1+r) 的 t 次方，r 上行时远期那几项被成倍地缩小，因此估值受损远比现金流靠前的价值股严重。",
  "CAPM 资本资产定价模型 `Ri = Rf + β(Rm − Rf)`。Rf 就是美债代表的无风险利率，是所有风险资产的收益基准。Rf 上涨 → 市场对股票、风险债券要求的最低回报 Ri 变高 → 同样的盈利只值更低的价格。高 β 成长股对 Rf 变动更敏感。",
  "why 2 年期美债紧跟美联储政策预期？2Y 属于短期债券，到期时间很近，机构买它主要就是赚政策利率这段收益。一旦市场预期美联储抬升利率，存量低息 2Y 立刻不值钱、遭抛售，收益率被动走高，所以 2Y 是观察政策预期最干净的窗口。"
 ]}
...
# 所以这些数字由配置算出来后替换进模板，不写死在文案里。
def _slots() -> dict[str, str]:
    return {
        "{{TOP_N}}": str(TOP_N),
        # 下限就是校验阈值，上限给个宽裕区间：链条太长反而啰嗦
        "{{HOPS_LO}}": str(MIN_CHAIN_HOPS + 1),
        "{{HOPS_HI}}": str(MIN_CHAIN_HOPS + 5),
        "{{NOTES_LO}}": str(MIN_NOTES + 1),
        "{{NOTES_HI}}": str(MIN_NOTES + 5),
    }
